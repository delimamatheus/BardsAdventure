if (titleVisible > 0)
{
	draw_sprite(sMenuTitle,0,0,-RESOLUTION_H + titleVisible * RESOLUTION_H);	
	
	draw_set_alpha(titleVisible * abs(sin(get_timer() * 0.000001 * pi)));
	draw_sprite(sMenuTitle,1,0,0);
	draw_set_alpha(1.0);
}
...
if (slotsVisible > 0)
{
	...
	for (var _slot = 0; _slot <= 2; _slot++)
	{
		...
		{
			_img = 3;
			draw_sprite(sMenuPointer,0,_x-32,_y+24);	
		}
		NineSliceBoxStretched(sTextBoxes,_x,_y,312,_y+48,_img);
